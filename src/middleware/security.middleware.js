...
const securityMiddleware = async (req, res, next) => {
  try {
    ...
    for (const result of decision.results) {
      logger.debug('Arcjet rule result', { id: result.id, action: result.action, reason: result.reason?.type });
    }
    ...
    if (decision.isDenied()) {
      ...
      if (decision.reason?.isShield()) {
        logger.warn('Shield blocked request', { ip: req.ip, ua: req.get('User-Agent'), path: req.path, method: req.method });
        return res.status(403).json({ error: 'forbidden', message: 'Request blocked by security policy' });
      }
      if (decision.reason?.isRateLimit()) {
        logger.warn('Rate limit exceeded', { ip: req.ip, ua: req.get('User-Agent'), path: req.path, role, limit: max });
        return res.status(429).json({ error: 'too_many_requests', message: msg });
      }

      // Fallback deny
      logger.warn('Request denied', { reason: decision.reason?.type || 'unknown' });
    ...
    res.status(500).json({ error: 'internal_error', message: 'Security middleware error' });
  }
};
